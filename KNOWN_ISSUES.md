...
## 14. ~~IPC 서버가 연결당 타임아웃/라인 길이 상한이 없음~~ — 해결됨 (2026-09-24, **실행 확인 완료**)
...
**수정 (2026-09-24)**: 값과 구조는 `docs/character-widget-design.md` "시간·길이 제한" 표에 정리했다. 사용자가 결정한 것:
- 줄 길이 상한은 8K자이고, 서버·클라이언트 **양쪽**에 적용한다. 상한을 넘으면 서버는 거절 응답을 보내고 끊는다.
- `CharacterTools`의 `CancellationToken.None`은 그대로 둔다. 클라이언트에 타임아웃이 생겨서 꼭 필요하지 않다.
- 연결당 타임아웃은 처음에 5초로 정했다가 **2초**로 바꿨다. 로직 테스트에서 클라이언트 연결 대기(3초)보다 길면, 멈춘 연결이 있는 동안 들어온 요청이
  "실행 중이지 않습니다"라는 틀린 안내를 받는 것을 확인했다. 클라이언트 대기를 6초로 늘리는 안과 비교한 뒤 서버 쪽을 줄였다.
  클라이언트 대기를 늘리면 흔한 경우(앱이 꺼져 있음)의 실패 안내가 6초로 늦어지기 때문이다.

구현하면서 밟을 뻔한 함정:
- **서버 루프 전체가 끝나는 문제.** 서버 루프는 `OperationCanceledException`을 받으면 `break`한다. 연결당 타임아웃도 같은 예외라서
  `when (!cancellationToken.IsCancellationRequested)`로 앱 종료와 구분하지 않으면, 타임아웃 한 번에 서버 루프가 끝난다.
- **`StreamWriter`의 잔여 버퍼.** 취소된 쓰기의 잔여 버퍼를 `Dispose`가 동기로 flush하려다가, 응답을 읽지 않는 상대 앞에서 다시 멈출 수 있다.
  그래서 응답은 바이트로 직접 쓴다.
- **크게 넘친 요청의 교착.** 20000자 요청에서 서버는 거절 응답을 쓰려 하고 클라이언트는 나머지를 보내려 해서 서로 멈췄다.
  서버 쓰기에 건 타임아웃이 풀어 줬지만 거절 이유는 전달되지 않았다 → 클라이언트가 보내기 전에 길이를 검사한다.

**로직 확인 (Claude, 앱 없이)**: `App.Core.dll`을 PowerShell로 불러, `app.log`를 스냅샷해 두었다가 원복하며 확인했다.
- 정상 왕복은 통과했다.
- 연결만 하고 아무것도 안 보내는 클라이언트는 2초 뒤 끊겼고, 그 사이 들어온 요청은 약 2초 뒤 성공했다.
- 8192자는 경계를 통과해 "요청 파싱 실패"가 나왔고, 8193자는 서버가 거절했다.
- 한글 대사 1,300자는 통과했고, 1,400자와 20,000자는 클라이언트가 보내기 전에 거절했다.
- `Stop()` 뒤 서버 루프는 정상 종료했다.
- 응답하지 않는 서버에 대해서는 5초 뒤 "응답하지 않습니다"가 나왔다.

**실행 확인 (2026-09-24, 6가지 모두 통과)**:
1. 기동 — `[ipc]` 오류가 없다.
2. 이중 실행 — 기존 창이 앞으로 나온다. 응답을 쓰는 방식이 바뀌었으므로 `activate` 회귀를 확인한 것이다.
3. MCP `say`/`set_expression("love")` — 말풍선과 표정이 반영된다.
4. 긴 대사 — 한글 1,358자는 통과하고 1,359자는 앱에 보내기 전에 거절된다(JSON 포장이 44자, 한글 1자가 6자).
   MCP로 보낸 "긴" 대사는 통과했는데, 생성하면서 글자 수를 정확히 셀 수 없어 경계보다 짧았던 것이다. 경계는 같은 `CharacterIpcClient`를 PowerShell에서 실제 앱에 보내 확인했다.
5. 멈춘 연결 — 연결만 잡은 클라이언트는 2017ms 만에 끊긴다. 그 사이 보낸 `say`는 2021ms 뒤 성공했고 말풍선도 떴다.
   도구 호출 사이의 지연 때문에 MCP로는 이 2초 창을 맞출 수 없어서, 같은 클라이언트로 한 스크립트 안에서 이어 보냈다.
   `app.log`에 `[ipc] 연결이 2000ms 안에 끝나지 않아 끊음` 두 줄이 남았다(실제 동작 기록이라 지우지 않음).
6. 앱 종료 후 `say` — "실행 중이지 않습니다"가 나온다.

**참고 — 말풍선은 200자에서 자른다**: IPC 상한(한글 약 1,358자)을 통과한 대사도 `SpeechBubbleWindow`가 `MaxChars = 200`에서 자르고 `…`를 붙인다.
너비는 260px로 고정이라 세로로 늘어나고, 작업 영역 안으로 맞춰져 시작 표시줄 근처에서 멈춘다. 두 상한은 역할이 다르다
(파이프 보호용과 화면 표시용). 의도된 동작이다.
...
### 17-1. ~~정렬 기준(마감일 우선 / 중요도 우선)을 설정에서 고르게 하는 안~~ — 해결됨 (2026-09-24, **실행 확인 완료**)
...
- **#27(컴포지션 루트)의 첫 단계를 여기서 한다**(2026-09-24 결정). 설정을 `MainViewModel`로 보내는 가지를 `MainWindow` 생성자에 또 붙이지 말고,
  서비스 조립 부분을 먼저 떼어낸 뒤 그쪽에서 연결한다.

**결정 (2026-09-24, 사용자)**:
- **선택지를 여는 것은 사분면 안의 순서뿐이다**(긴급 먼저 / 중요 먼저). 마감일이 사분면보다 먼저인 규칙은 고정했다. 사분면을 먼저 보게 하면
  딱지 붙은 먼 일이 오늘 마감 위로 올라와 마감을 놓치게 만들고, 선택지가 2×2가 되면 설정창에서 설명하기 어렵다.
- **기본값은 긴급 먼저(기존 동작)**. 사용자가 짚은 근거: 매트릭스가 3사분면(급하지만 중요하지 않음)을 뒤로 미룰 수 있는 건 맡길 사람이 있어서인데,
  이 앱의 사용자(프리랜서, 개인사업자)는 맡길 곳이 없다. 2사분면 처방인 "일정을 잡아라"는 우리 앱에서는 마감일을 붙이는 것이고, 마감일은 사분면보다 먼저 본다.
  중요 먼저는 "긴급함의 함정"에 빠지기 쉬운 사용자를 위한 선택지로 연다. 상세 근거는 `docs/todo-design.md` "목록 정렬 규칙".
- **설정창 표시**: 맨 위에 "할 일" 절을 둔다. 라벨은 목록 딱지와 같은 단어("긴급 먼저" / "중요 먼저")를 쓰고, `중요`는 주황, `긴급`은 빨강으로 딱지와 색을 맞춘다.
  "(기본)" 표시는 선택을 기울게 해서 넣지 않았다. "마감일 없는 할 일끼리도 이 순서를 따른다"는 설명은 오해를 막으려고 넣었다.

**구현 (2026-09-24)**: `TodoSortOrder` enum과 `TodoSortKey.From(item, order)`를 추가했다. `AppSettings.TodoSortOrder`는 `JsonStringEnumConverter`로 이름으로 저장하고,
필드가 없는 예전 파일은 긴급 먼저로 읽힌다. 정렬 기준은 `MainViewModel`만 안다 — `KeyOf`를 `LoadItems`와 `UpsertItem`이 공유하고,
`TodoItemViewModel`은 계산된 키를 받기만 한다. 설정이 바뀌면 `ApplySortOrder`가 `LoadItems`로 VM을 전부 새로 만든다.
그래서 위에 적은 "정렬 키가 VM 생성자에서 굳는 문제"가 생기지 않는다. 배선은 #27 첫 단계(`AppServices`) 위에서 했다.

**검증 (2026-09-24)**: 하네스 8가지를 확인했다(전부 통과) — 두 모드의 순서, 마감일이 사분면보다 먼저, 딱지 없는 항목과 둘 다 붙은 항목은 모드와 무관하게 키가 같음,
예전 파일은 긴급 먼저로 읽히고 다른 값이 유지됨, 이름으로 저장됨, 모르는 이름은 `JsonException`(기존 복구 경로).

**실행 확인 (2026-09-24, 사용자)**: 모두 통과했다.
- 두 모드의 목록 순서가 예상과 일치했다(오늘 마감 4사분면, 내일 마감, 마감 없음까지).
- 중요 먼저에서 새로 추가한 "오늘+중요" 항목이 기존 오늘-중요 아래, 오늘-긴급 위에 들어갔다(`UpsertItem`이 새 기준의 키로 끼워 넣음).
- 재시작 후에도 유지됐다. 체크리스트 체크와 할 일 완료가 다른 항목의 순서를 흔들지 않았다.
- 바꾸지 않고 저장해도 스크롤이 그대로였다(7회 시도).
- `settings.json`에 `"TodoSortOrder": "ImportantFirst"`로 저장됐다.

**후속 아이디어 (사용자)**: 지금 설정창은 필요한 내용만 있어 줄일 곳이 없다. UI 편집 기능 같은 것이 들어오면 설정창을 **탭으로 나누는 것**을 고려한다.
...
**실행 확인 정정 (2026-09-24)**: 처음 확인(아래 문단)은 **9/22 빌드로 돌아 무효였다.** 컴파일 확인을 스크래치패드 출력으로만 하고
실행 폴더(`App.Windows/bin`)에는 빌드하지 않았기 때문이다. 게다가 그 시나리오들은 옛 코드에서도 모두 통과하는 것이라 무효인 줄 몰랐다.
특히 함정 1(삭제 후 되살아남)은 새 코드가 `Closing`에서 저장하기 때문에 생기는 위험이라, 옛 코드로는 확인이 안 된다.
#17-1 설정창에 새 절이 안 보인다는 사용자 보고로 발견했고, `App.Windows`를 빌드한 뒤(DLL 안에 새 타입이 있는지 확인) **같은 6가지를 다시 확인해 모두 통과했다.**

**실행 확인 (2026-09-24, 사용자, 무효 — 위 정정 참고)**: 6가지 통과 — 이동/리사이즈 후 재시작, 이동 직후 바로 종료, 최소화한 채 종료(#20 회귀 없음),
...
## 22. ~~단일 인스턴스 뮤텍스는 세션 단위인데 IPC 파이프는 머신 전역~~ — 해결됨 (2026-09-24, **실행 확인 완료**)
...
**수정 (2026-09-24)**: 사용자가 (1) 접미사는 **세션 ID**, (2) **`PipeOptions.CurrentUserOnly`도 같이** 넣기로 결정했다. #14(타임아웃·줄 길이 상한)는 같은 파일이지만 확인 범위를 좁히려고 따로 한다.
- `CharacterIpcContract.PipeName`이 `const`에서 `static` 속성으로 바뀌었다. Windows는 `StickyGhost.CharacterIpc.<세션 id>`이고, 그 밖의 OS는 접미사가 없다.
  - 세션 ID를 고른 이유: 뮤텍스 `Local\`과 범위가 정확히 같다. 사용자 SID로 하면 같은 사용자가 두 세션에 로그인했을 때 뮤텍스와 범위가 어긋난다.
  - Mac에 접미사가 없는 이유: `Process.SessionId`가 Unix에서는 POSIX 세션이라 앱과 `App.Mcp`의 값이 다를 수 있다. 그리고 파이프 소켓이 사용자별 TMPDIR에 생긴다. 상세는 `docs/PORTING.md` "IPC 파이프 이름"(미확인).
- 서버·클라이언트 모두 `CurrentUserOnly`를 켰다. 세션 번호는 예측할 수 있어서, 이름만으로는 다른 사용자가 같은 이름을 먼저 잡는 것을 막지 못한다.
  클라이언트는 주인이 다를 때 나는 `UnauthorizedAccessException`을 잡아 안내 문구로 돌려준다(로그는 남기지 않음 — `App.Mcp`가 `app.log`를 오염시키지 않게. 두 번째 인스턴스 쪽은 `SingleInstanceGuard`가 응답 실패를 이미 로그로 남긴다).
- **알려진 제약**: 앱만 **관리자 권한**으로 실행하면 파이프 주인이 Administrators 그룹이 될 수 있다. 이 경우 일반 권한의 `App.Mcp` 연결이 거부될 수 있다(위 안내 문구가 나옴).
- **업그레이드 주의**: 옛 `App.Mcp`는 옛 이름으로 접속하므로 `App.Mcp`도 다시 빌드하고 MCP를 재시작해야 한다.

**로직 확인 (Claude, 앱 없이)**: `App.Core.dll`을 PowerShell로 불러 확인했다.
- 이름이 `StickyGhost.CharacterIpc.14`(현재 세션 14)로 나왔다.
- `CurrentUserOnly`를 켠 서버·클라이언트가 두 번 연속 왕복에 성공했다.
- 서버가 없을 때 3초 뒤 "실행 중이지 않습니다"를 돌려줬다.
- 두 사용자가 동시에 쓰는 상황은 재현하지 않았다(재현 수단 없음).

**실행 확인 (2026-09-24, 5가지 모두 통과)**:
1. 기동 — 떠 있는 파이프는 `StickyGhost.CharacterIpc.14` 하나뿐이고 `app.log`에 `[ipc]` 오류가 없다.
2. 이중 실행 — 기존 메인 창이 앞으로 나온다(새 이름으로 바뀐 `activate` 경로).
3. MCP `say`/`set_expression("love")` — 말풍선과 표정이 반영됐다(사용자가 화면으로 확인).
4. 앱을 끈 뒤 `say` — "캐릭터 위젯이 실행 중이지 않습니다"가 돌아온다.
5. 앱을 **관리자 권한**으로 실행한 뒤 `say` — "파이프의 주인이 현재 사용자가 아닙니다" 안내가 돌아온다. 서버는 정상이고, 클라이언트의 주인 확인 단계에서 거부된 것이다.
   - 이 상태에서 일반 권한으로 한 번 더 실행하면 창 활성화도 같은 이유로 실패한다. `[single-instance]` 로그 한 줄이 남고 두 번째 프로세스는 조용히 종료한다.
   - 수정 **전**에 관리자 권한 앱과 MCP가 연결됐는지는 확인하지 않았다. Windows 기본 파이프 권한상 수정 전에도 일반 권한 클라이언트의 쓰기가 막혔을 가능성이 있다.
...
## 27. 앱 전체의 조립(컴포지션 루트)이 `MainWindow` 생성자에 있음 — **첫 단계 완료**, 나머지는 필요해질 때마다 (2026-09-24)

**어디**: `App.UI/Views/MainWindow.axaml.cs`의 생성자와 `static HttpClient` 필드, `BuildReactionService`.

**무엇**: 진입점(`App.Windows/Program`)은 `ISecretStore`/`IWindowBehavior`만 만들어서 넘긴다. 나머지 조립은 창 하나인 `MainWindow`가 전부 직접 한다 —
데이터 폴더, `WindowStateStore`, SQLite 스키마 초기화, 리포지토리 3개, `TodoService`와 `TodoEventBus` 구독,
캐릭터 팩 서비스, 설정 저장소, 캐릭터 오버레이, IPC 서버, LLM 어댑터 조립.
그래서 **앱 전체의 수명이 창 하나에 묶여 있다**: 서비스·IPC·구독이 `MainWindow`가 만들고 `MainWindow`가 닫힐 때 같이 정리된다.

**경위**: 기능을 제대로 굴러가게 만드는 데 집중하는 동안 조립이 한 곳에 쌓였다. 2026-09-22 배선 점검 때 후보가 셋이었는데
(잡음 정리 / #15 이벤트 버스 연결 / 컴포지션 루트 분리), 사용자가 **잡음 정리만** 골라 `4cda389`에서 처리했고, 이벤트 버스는 이후 #15-1에서 연결됐다.
이 항목만 구조 변경이라 보류됐다. `HttpClient`를 그대로 둔 것도 옮기려면 이 분리가 먼저라서다.

**왜 지금은 안 급한가**:
- 알려진 버그를 일으키지 않는다.
- 조립 코드가 `App.UI`에 있어 Mac 진입점도 그대로 재사용할 수 있다 — 이식 자체를 막지 않는다.
- 한 번에 옮기면 IPC, 메모 창, 캐릭터 오버레이, 종료 순서(`OnClosing`의 저장 flush)를 한꺼번에 건드리게 되어 실행 확인 범위가 너무 넓다.

**분리가 필요해지는 계기**(이 중 하나를 만들 때 그 부분만 풀어낸다):
1. **#17-1 정렬 기준 설정** — 설정창에서 바꾼 값이 `MainViewModel`까지 가야 하는데, 지금은 `MainWindow`를 한 번 더 거쳐야 한다.
2. **#15의 남은 부분(마감 임박 알림)과 캐릭터가 먼저 말을 거는 능동 알림** — `CheckDueSoon`을 주기적으로 부를 스케줄러가 필요한데, 이건 창에 속할 물건이 아니다.
3. **트레이 상주** — 메인 창을 닫아도 앱이 살아 있어야 하므로 이 기능을 넣으면 분리가 **필수**다.

**진행 방식 (사용자 결정, 2026-09-24)**: 날을 잡아 한 번에 옮기지 않고, **필요해질 때마다 조금씩 풀어낸다.**
- **지금부터의 규칙**: 새로 만드는 서비스는 `MainWindow` 생성자에 더 쌓지 않는다.
- **첫 단계는 #17-1 논의 때**: 서비스 조립 부분만 `AppServices` 같은 클래스로 떼어내고, 창은 완성된 것을 받게 한다.
- **수명 분리**(창이 닫혀도 서비스가 사는 구조)는 트레이 상주나 알림 스케줄러를 만들 때 한다.

**첫 단계 (2026-09-24, #17-1과 함께)**: `App.UI/Composition/AppServices.cs`를 새로 만들었다. 저장소, 도메인 서비스, 캐릭터 팩 서비스, `HttpClient`는
`AppServices.Create`가 만들고, `App.axaml.cs`가 메인 창에 넘긴다. `HttpClient`는 `MainWindow`의 `static` 필드에서 `AppServices`가 소유하는 인스턴스로 바뀌었다.
창·오버레이·버스 구독·IPC 서버·메모 창처럼 창이나 UI에 기대는 것은 `MainWindow`에 남겼다. **수명은 아직 메인 창과 같다.**
**실행 확인 (사용자)**: 동작 변화가 없는지 회귀 6가지를 확인해 모두 통과했다 — 기동, 완료 반응, 쓰다듬기 LLM 반응, 설정 즉시 반영, 이중 실행 시 창 활성화, MCP `say` 말풍선.
**남은 것**: 수명 분리(트레이 상주나 마감 알림 스케줄러를 만들 때).

**트레이 상주는 강제하지 않고 설정으로 고르게 한다 (사용자, 2026-09-24)**: 닫기 버튼을 눌렀을 때
(a) 앱을 완전히 종료할지(지금 동작), (b) 창만 닫고 트레이에 남을지를 설정창에서 고르게 한다. 기본값은 그 기능을 만들 때 정한다.
(b)를 넣으려면 `App.axaml.cs`의 `ShutdownMode.OnMainWindowClose`를 명시적 종료(`OnExplicitShutdown`) + 트레이 메뉴의 "종료"로 바꿔야 한다.
**이 전환이 아래 주의점이 가장 크게 드러나는 지점이다.**

**분리할 때의 주의점 — 고아 창/프로세스 (사용자, 2026-09-24)**: 개발 중에 오버레이 창이 메인 창과 수명이 연결되지 않아
**메인 창을 닫아도 고아로 남고, 작업 관리자로 프로세스를 강제 종료해야 했던 적이 몇 번 있었다**(문서에는 남아 있지 않은 사례).
#27은 `MainWindow`가 붙잡고 있는 것을 조금씩 바깥으로 옮기는 작업이라, 옮길 때마다 이 위험이 커진다.
- **지금의 안전망은 두 개다.** `ShutdownMode.OnMainWindowClose`(메인 창이 닫히면 프로세스 종료)와 `MainWindow.OnClosing`의 정리
  (메모 저장 flush, 캐릭터 위치 flush, IPC 서버 `Stop`)다. 무언가를 `MainWindow` 밖으로 옮기면 **그것을 닫고 멈출 책임도 함께 옮겨야 한다.**
  만든 곳이 바뀌었는데 정리는 여전히 `OnClosing`에 기대고 있으면, 그 경로가 끊기는 순간 고아가 된다.
- **트레이 모드에서는 첫 번째 안전망이 사라진다.** 메인 창이 닫혀도 프로세스가 살아 있어야 하기 때문이다. 그때는 앱 수준의 종료 경로를 **하나**로 만들고
  (예: `AppServices` 쪽 종료 메서드), 모든 창 닫기, IPC 서버와 타이머 멈추기, 저장 flush를 거기서 한다. 창이 하나라도 이 경로 밖에서 만들어지면 프로세스가 끝나지 않는다.
...
## 28. 중간점검에서 나온 코드 간 모순 (2026-09-24) — A 전부 · B-6~B-9 해결·**실행 확인 완료**, B-10·C는 논의 예정
...
**남은 것 (미해결)**:
- ~~B-6: 두 LLM 어댑터에 시스템 프롬프트(인젝션 방어 문구)와 응답 파싱이 한 벌씩 복사되어 있음~~ — **해결 (2026-09-24, 실행 확인 완료)**.
  `Infrastructure/Llm/LlmReactionPrompt`(internal)로 모음: `BuildSystemPrompt`, `ParseReactionContent`. 응답 스키마 방언·봉투 위치·인증·HTTP 판정은 어댑터에 남김
  (HTTP 흐름까지 합치면 Gemini 400 판정 때문에 끼워 넣기 구조가 필요해 오히려 읽기 어려워진다). 가짜 `HttpMessageHandler`로 전후 72가지(요청 본문·URL·키 헤더, 정상/깨진 응답, HTTP 오류)를 녹화·비교해 **바이트 단위 동일** 확인.
- ~~B-7: IPC 요청 종류 `"say"`/`"setExpression"`이 상수 없이 문자열로 두 프로젝트에 있음~~ — **해결 (2026-09-24, 실행 확인 완료)**. `CharacterIpcRequest.TypeSay`/`TypeSetExpression` 상수. 파이프로 오가는 값은 그대로라 옛 App.Mcp 프로세스와도 통신된다.
- ~~B-8: 캐릭터 팩 스캐너를 두 곳에서 따로 만들고, 설정창을 닫을 때마다 팩 전체를 두 번 훑음~~ — **해결 (2026-09-24, 실행 확인 완료)**. `AppServices.PackScanner` 하나를 오버레이와 설정창이 같이 쓰고, 로더도 팩 서비스와 공유.
  고른 팩이 없으면 `Apply`가 스캔을 건너뛴다(결과는 같음, 그 경우의 "목록에서 제외" 로그는 설정창을 열 때 남는다 — 사용자 결정).
