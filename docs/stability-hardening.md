...
| 실제 실행 검증 — C1 이중 실행 | **확인됨(사용자, 2026-09-20)**: 최소화된 메인 창이 두 번째 실행으로 복원되어 앞으로 나옴 |
| 실제 실행 검증 — C2 손상 팩 | **확인됨(사용자, 2026-09-20)**: 기동 시 창 생성 경로(스택에 `CharacterWindow..ctor`)와 창이 떠 있는 상태의 교체 경로(스택이 `ApplyFolder → SetPack`)에서 모두 크래시 없이 안내 다이얼로그 + `app.log`의 `[pack]` 기록. 교체 실패 시 기존 캐릭터가 화면에 그대로 남는지는 별도로 보고받지 않음 |
| 실제 실행 검증 — C3, G | **미확인.** `Unexpected` 경로와 전역 핸들러는 재현이 어려워 코드 리뷰 수준 |
| 실측 중 발견 | 팩 폴더를 복사하고 `id`를 안 바꾸면 팩 선택이 동작하지 않음 — 이번 변경의 회귀는 아니며 `KNOWN_ISSUES.md` #11에 기록 |
| 커밋 | `ad36343`(코드 + 문서 — 이 변경만 한 커밋이라 `git revert`로 통째로 되돌릴 수 있다). 이후 문서 정리·검증 결과 반영은 별도 커밋 |
...
2. **손상 팩 폴백 (C2)**: 위 경로의 `CharacterPacks\`에 `default` 폴더를 복사해 `broken`으로 만들고 `manifest.json`의 `id`를 **`default`가 아닌 다른 값**
   (예: `broken`)으로 바꾼다(id가 겹치면 `KNOWN_ISSUES.md` #11로 선택이 동작하지 않는다). 그런 다음 `broken\mainimage.png`를 **PNG 헤더는 정상이고 본문만
   깨진 파일**로 만든다 — 예: 정상 PNG의 앞 33바이트(시그니처 + IHDR)만 남기고 잘라 낸다(PowerShell:
   `$b=[IO.File]::ReadAllBytes($p); [IO.File]::WriteAllBytes($p,$b[0..32])`, Skia 디코더로 실패함을 확인함). → 앱 실행 → 설정에서 `broken` 선택 후 저장 →
   기본 캐릭터가 표시되고 안내 다이얼로그가 떠야 하며 `app.log`에 `[pack]`이 남아야 한다. 재실행해도 크래시 없이 같은 안내가 뜬다.
   (2026-09-20 이후 로더가 PNG 헤더를 검사하므로, 예전처럼 텍스트 파일을 `.png`로 바꾼 팩은 이 폴백까지 오지 않고 스캔 단계에서 목록에서
   제외된다 — 사유는 `app.log`의 `[pack] 팩 목록에서 제외`.)
