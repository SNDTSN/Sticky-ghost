...
## 목록 정렬 규칙 (확정 2026-09-20, 사분면 순서 선택 추가 2026-09-24)

미완료 목록(`MainWindow`의 `ListBox`)은 다음 순서로 정렬한다. 규칙은 `src/App.Core/Domain/Services/TodoSortKey.cs` 한 곳에만 있다.

1. **마감일 — 날짜 단위**. 마감일이 없는 항목은 맨 뒤.
2. **아이젠하워 사분면 — 설정에서 선택**(`AppSettings.TodoSortOrder`):
   - 긴급 먼저(기본값): 중요+긴급 → 긴급 → 중요 → 없음.
   - 중요 먼저: 중요+긴급 → 중요 → 긴급 → 없음.
3. **생성순**(`CreatedAt`).
...
**왜 사분면 순서를 고르게 했나 (2026-09-24, KNOWN_ISSUES #17-1)**: 처음에는 긴급 우선으로 고정했다(2026-09-20). 같은 날 안에서는
"지금 뭐부터?"에 바로 답하는 쪽이 낫다고 봤기 때문이다. 그런데 이건 사람마다 갈리는 취향이다.
아이젠하워 매트릭스 본래 의도는 중요 우선이다 — 급하지만 중요하지 않은 일(3사분면)은 남에게 맡기고,
중요하지만 급하지 않은 일(2사분면)은 일정을 잡아 직접 챙긴다.

- **기본값을 긴급 먼저로 둔 이유**: 이 앱의 사용자는 주로 프리랜서나 개인사업자라 3사분면 일을 맡길 곳이 없다. 그러면 그 일도 결국 본인이 해야 하고 시간 압박이 있다.
  그리고 매트릭스가 2사분면에 주는 처방("일정을 잡아라")은 우리 앱에서 마감일을 붙이는 것이고, 마감일은 사분면보다 먼저 본다.
  마지막으로 기존 사용자의 순서가 바뀌지 않는다.
- **중요 먼저를 선택지로 연 이유**: "긴급함의 함정"(급한 잡무가 중요한 일을 계속 밀어냄)은 혼자 일하는 사람에게 더 잘 일어난다.
  긴급 먼저에서는 마감일 없는 중요 항목이 가라앉기 쉽다.
- **1번(마감일 먼저)은 고정이다**: 사분면을 마감일보다 위에 두는 선택지는 열지 않았다. 딱지 붙은 먼 일이 오늘 마감 위로 올라와 마감을 놓치게 만든다.

정렬 기준은 `MainViewModel`만 안다. `TodoItemViewModel`은 계산된 키를 받기만 한다. 설정이 바뀌면 `MainViewModel.ApplySortOrder`가
`LoadItems`로 VM을 전부 새로 만든다. 그래서 옛 기준으로 계산한 키가 목록에 남아 `UpsertItem`의 삽입 위치 탐색을 어긋나게 하는 일은 없다.

**키가 전순서라는 점이 중요하다**: `Id`까지 비교하므로 서로 다른 항목의 키는 절대 같지 않다. 그래서 목록에 항목 하나를 끼워 넣을
자리가 유일하게 정해지고, 증분 갱신(`MainViewModel.UpsertItem`)이 만든 순서가 전체 재정렬(`LoadItems`)의 결과와 항상 일치한다.
덕분에 `GetIncomplete()`에 `ORDER BY`를 두지 않아도 되고(DB 스캔 순서에 의존하지 않는다), "재시작하면 순서가 달라진다"는 문제가 생길 수 없다.

**정렬 키가 바뀌지 않는 갱신은 자리를 옮기지 않는다**: `UpsertItem`은 기존 항목과 키가 같으면 제거 후 재삽입이 아니라 제자리 교체를 한다.
체크리스트 토글은 아예 목록을 건드리지 않는다(저장만) — 자세한 경위는 `KNOWN_ISSUES.md` #17.
...
- [x] To-do 리스트 3차 — UX 개편 (2026-09-19, `MainWindow.axaml`/`.axaml.cs`, `MainViewModel`, `TodoItemViewModel`).
  ...
  - **버그 발견 및 수정**: `ITodoRepository`에 애초에 `Delete`가 없어서 반복 종료일 없는 항목을 영원히
    ...
    삭제 전 확인창을 띄우게 함.
- [x] To-do 리스트 4차 — 목록 정렬 규칙 확정 (2026-09-20, `TodoSortKey` 신규 + `MainViewModel`/`TodoItemViewModel`).
  체크리스트를 체크하면 그 항목이 목록 아래로 밀려나는 버그(`KNOWN_ISSUES.md` #17)를 고치면서, 흩어져 있던 정렬 규칙을
  `TodoSortKey` 한 곳으로 모으고 아이젠하워 사분면을 정렬에 반영함. 규칙과 근거는 위 "목록 정렬 규칙" 참고.
  **실행 확인 필요** — 체크 시 자리 유지, 같은 날 사분면 순서, 재시작 후 순서 동일, 반복 항목 완료 시 이동.
