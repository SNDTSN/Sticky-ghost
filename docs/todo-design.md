...
2. **아이젠하워 사분면 — 긴급 우선**: 중요+긴급 → 긴급 → 중요 → 없음.
3. **생성순**(`CreatedAt`).
...
**왜 긴급 우선인가**: 아이젠하워 매트릭스 본래의 우선순위는 중요 우선(중요+긴급 → 중요 → 긴급 → 없음)이지만,
같은 날 안에서는 "지금 뭐부터?"에 바로 답하는 쪽이 낫다고 보고 긴급 우선을 택했다(2026-09-20 결정).
바꾸려면 `TodoSortKey.QuadrantOf`의 두 가중치를 맞바꾸면 된다.
